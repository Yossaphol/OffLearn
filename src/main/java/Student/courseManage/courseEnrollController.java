...
public class courseEnrollController implements Initializable {
    ...
    private void setEffect(){
        ...
        ef.hoverEffect(addToCartBtn);
        ef.hoverEffect(enrollBtn1);
        ef.hoverEffect(otherCourse);
    }
    ...
    public void setCourseDetail(String name, String _shortDescription, String _description, String picPath, String categoryPicPath, String _category, double _price, double _rating, int totalReview, int _totalLesson, int _duration){
        setRating(_rating);
        couseName.setText(name);
        shortDescription.setText(_shortDescription);
        description.setText(_description);
        totalLesson.setText(String.valueOf(_totalLesson)+" บท");
        duration.setText(String.valueOf(_duration)+" ชม.");
        price.setText(String.valueOf(_price)+" บาท");
        rating.setText(String.valueOf(_rating)+" ("+String.valueOf(totalReview)+")");
        category.setText(_category);
        ef.loadAndSetImage(courseImg, picPath);
        ef.loadAndSetImage(categoryPic, categoryPicPath);
    }


    private void setDetail(){
        adviceText.setText("เนื้อหาที่ปูพื้นฐานให้ตั้งแต่เริ่มต้น\nตั้งแต่กระบวนการคิด จนถึงการปฏิบัติ");
        label1.setText("แนวคิด");
        label2.setText("ปฏิบัติ");
        label3.setText("โปรเจคต์");

        reviewerName1.setText("Bobby");
        reviewerName2.setText("John");
        reviewerName3.setText("Jeffy");
    }
