...
public class CourseDB extends ConnectDB{
    ...
    public ArrayList<CourseItem> getUserCourses(int userId) {
        ArrayList<CourseItem> courseList = new ArrayList<>();

        String sql = "SELECT c.Course_ID, c.courseName, c.image, c.price, c.courseDescription, " +
                "cat.catname, CONCAT(u.firstname, ' ', u.lastname) AS teacherName " +
                "FROM offlearn.course c " +
                "JOIN offlearn.category cat ON c.Cat_ID = cat.Cat_id " +
                "JOIN offlearn.user u ON c.User_ID = u.User_ID " +
                "JOIN offlearn.enroll e ON c.Course_ID = e.Course_ID " +
                "WHERE e.User_ID = ? AND c.verify = 1";
